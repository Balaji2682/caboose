...
fn render_header(f: &mut Frame, area: Rect, group: &ExceptionGroup, severity: ExceptionSeverity, fade_progress: Option<f32>) {
    ...
    let header_text = vec![
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(Theme::apply_fade_to_color(severity_color, fade_progress.unwrap_or(1.0))).add_modifier(Modifier::BOLD),
            ),
            Span::raw("  "),
            Span::styled(
                format!("[{}]", severity.icon()),
                Style::default().fg(Theme::apply_fade_to_color(severity_color, fade_progress.unwrap_or(1.0))),
            ),
        ]),
        Line::from(""),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::raw(format!(
                "{}:{}",
                group.sample_exception.file_path.as_ref().unwrap_or(&"unknown".to_string()),
    ...
    let paragraph = Paragraph::new(header_text)
        .block(
            Theme::block(" Exception Details ", fade_progress)
                .border_style(Style::default().fg(Theme::apply_fade_to_color(Theme::text_secondary(), fade_progress.unwrap_or(1.0))))
        )
        .wrap(Wrap { trim: true });
...
fn render_backtrace(f: &mut Frame, area: Rect, exception: &crate::exception::Exception, fade_progress: Option<f32>) {
    ...
    let title = if backtrace_count > 20 {
        format!(" Backtrace (showing 20 of {}) - Press Esc to go back ", backtrace_count)
    } else {
        " Backtrace - Press Esc to go back ".to_string()
    };

    let list = List::new(backtrace_items).block(
        Theme::block(title, fade_progress)
            .border_style(Style::default().fg(Theme::apply_fade_to_color(Theme::text_secondary(), fade_progress.unwrap_or(1.0))))
    );
