use portable_pty::{CommandBuilder, PtySize, native_pty_system, ChildKiller};
    ...
    fn kill(&self) -> Result<(), String> {
        match self {
            ChildHandle::Pty { killer, child } => {
                ...
                mutable_killer.kill().map_err(|e| format!("Failed to kill PTY child: {}", e))?;
                // Best-effort reap without holding the lock long
                let _ = child.lock().map_err(|_| "Failed to lock PTY child".to_string())?.try_wait();
                Ok(())
            }
            ChildHandle::Plain { child } => {
                let mut child = child.lock().map_err(|_| "Failed to lock process".to_string())?;
...
impl ProcessManager {
    ...
    pub fn spawn_process(&self, name: String, command: String, env_vars: HashMap<String, String>) -> Result<(), String> {
    ...
    fn spawn_with_pty(&self, name: String, command: String, env_vars: HashMap<String, String>) -> Result<(), String> {
        ...
        // Track child handle for cleanup
        {
            ...
            handles.insert(name.clone(), ChildHandle::Pty {
                killer,
                child: child.clone(),
            });
        }
    ...
    fn spawn_without_pty(&self, name: String, command: String, env_vars: HashMap<String, String>) -> Result<(), String> {
        // Parse command into parts
        let parts: Vec<&str> = command.split_whitespace().collect();
        if parts.is_empty() {
            return Err("Empty command".to_string());
        ...
        {
            let mut handles = self.child_handles.lock().unwrap();
            handles.insert(name.clone(), ChildHandle::Plain { child: child.clone() });
        }
    ...
    pub fn stop_all(&self) {
        let handles: Vec<(String, ChildHandle)> = {
            ...
            handles.iter().map(|(name, handle)| (name.clone(), handle.clone())).collect()
        };
