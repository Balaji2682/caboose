...
impl DatabaseHealth {
    ...
    pub fn get_issues(&self) -> Vec<DatabaseIssue> {
        ...
        if stats.select_star_count > 5 {
            issues.push(DatabaseIssue {
                ...
                description: "SELECT * queries fetch all columns, which can be inefficient.".to_string(),
        ...
        for sq in slow_queries.iter().take(5) {
            if sq.duration > 500.0 {
                let table_hint = sq.table.as_ref().map_or(String::new(), |t| format!(" on table '{}'", t));
    ...
    pub fn calculate_health_score(&self) -> u32 {
        ...
        if stats.total_queries > 0 {
            let select_star_ratio = (stats.select_star_count as f64 / stats.total_queries as f64) * 100.0;
            if select_star_ratio < 5.0 {
                score = (score + 5).min(100);
            }

            let slow_query_ratio = (stats.slow_queries_count as f64 / stats.total_queries as f64) * 100.0;
    ...
    pub fn get_top_tables(&self) -> Vec<(String, usize)> {
        let stats = self.query_stats.lock().unwrap();
        let mut tables: Vec<_> = stats.tables_accessed.iter()
            .map(|(k, v)| (k.clone(), *v))
            .collect();
