...
impl ExceptionSeverity {
    pub fn from_exception_type(exc_type: &str) -> Self {
        match exc_type {
            ...
            // High severity
            "NameError" | "NoMethodError" | "ArgumentError" | "TypeError" |
            "ZeroDivisionError" | "SyntaxError" => Self::High,

            // Medium severity
            "ActiveRecord::RecordNotFound" | "ActiveRecord::RecordInvalid" |
            "ActionController::RoutingError" | "ActiveRecord::RecordNotUnique" => Self::Medium,
...
impl ExceptionTracker {
    ...
    fn finalize_current_exception(&self) {
        ...
        if let Some(exception) = current.take() {
            ...
            if let Some(group) = grouped.get_mut(&fingerprint) {
                ...
                grouped.insert(fingerprint.clone(), ExceptionGroup {
    ...
    fn normalize_message(message: &str) -> String {
        ...
        normalized = regex::Regex::new(r"\d+").unwrap()
    ...
    pub fn get_critical_exceptions(&self) -> Vec<ExceptionGroup> {
        self.get_grouped_exceptions()
            .into_iter()
            .filter(|g| {
                ExceptionSeverity::from_exception_type(&g.exception_type) == ExceptionSeverity::Critical
            })
            .collect()
    }
