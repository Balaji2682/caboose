...
fn temp_dir(name: &str) -> PathBuf {
    ...
    let uniq = format!("caboose_frontend_{}_{}", name, std::time::SystemTime::now().elapsed().unwrap().as_millis());
    dir.push(uniq);
    dir
}
...
fn detects_vite_frontend_and_package_manager() {
    ...
    assert!(app.generate_procfile_entry().unwrap().contains("yarn run dev"));
