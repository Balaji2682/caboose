...
fn temp_dir(name: &str) -> PathBuf {
    ...
    let uniq = format!("caboose_rails_{}_{}", name, std::time::SystemTime::now().elapsed().unwrap().as_millis());
    dir.push(uniq);
    dir
}
...
fn detects_rails_app_and_features() {
    ...
    fs::write(root.join("Gemfile"), "gem 'rails'\ngem 'sidekiq'\ngem 'vite_rails'").unwrap();
    ...
    assert!(app.generate_procfile().contains("bundle exec sidekiq"));
